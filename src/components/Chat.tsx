...
import { Send, Paperclip, File, Image, Download, X, Smile, Search, Users, MessageSquare } from 'lucide-react';
...
const mockContacts: Contact[] = [
  ...
  { id: '3', name: 'Carol Davis', avatar: 'CD', status: 'online', lastMessage: 'Meeting at 3pm?' },
            ...
            <div className={`p-4 border-b flex items-center gap-3 ${darkMode ? 'border-white/[0.08] bg-white/[0.02]' : 'border-gray-200 bg-white'}`}>
              ...
              <div>
                ...
                <p className={`text-xs ${currentContact.status === 'online' ? 'text-green-500' : darkMode ? 'text-white/40' : 'text-gray-500'}`}>
                  {currentContact.status === 'online' ? 'Online' : currentContact.status === 'away' ? 'Away' : 'Offline'}
                </p>
              </div>
            </div>
